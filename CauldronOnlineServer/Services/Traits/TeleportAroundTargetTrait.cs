...
{
    public class TeleportAroundTargetTrait : WorldTrait
    {
        ...
        public TeleportAroundTargetTrait(WorldTraitData data) : base(data)
        {
            if (data is TeleportAroundTargetTraitData teleport)
            {
        ...
        public override void Setup(WorldObject parent, object sender)
        {
            ...
            this.SendMessageTo(new QueryTargetMessage{DoAfter = id => targetId = id }, _parent);
            if (!string.IsNullOrEmpty(targetId))
            {
                var zone = ZoneService.GetZoneById(_parent.ZoneId);
                if (zone != null)
                {
                    if (zone.ObjectManager.TryGetObjectById(targetId, out var target))
                    {
                        var tiles = zone.GetTilesInPovArea(target.Tile, _range);
                        if (tiles.Length > 0)
                        {
